# View: vw_spigafacturasProveedores
...
```sql
CREATE VIEW [dbo].[vw_spigafacturasProveedores]  AS	
select Id_Empresa,Nombre_Proveedor,Nit,	Numero_Factura,Factura,Moneda,Fecha_Factura,Valor_Total,IVA,ReteFuente,ReteIva,ReteIca,
...
from(
	select distinct Id_Empresa=f.IdEmpresas,Nombre_Proveedor=f.NombreTercero,Nit=case when len(t.NifCif) = 10 then substring(t.NifCif,1,9) else t.NifCif end,
	Numero_Factura=f.Factura,f.Factura,Moneda=f.DescripcionMoneda,Fecha_Factura=i.FechaFactura,Valor_Total=i.TotalDocumento,IVA=isnull(i.IVA,0),
	...
	Banco_Receptor=b.Nombre,Cuenta_Bancaria=b.CuentaNumero,Codigo_Spiga=f.IdTerceros,
	Estado=case when f.DescripcionSituacionEfectos = 'Cancelado hoja de gastos'
	...
	UNION ALL
	select distinct Id_Empresa=f.IdEmpresas,Nombre_Proveedor=f.NombreTercero,Nit = case when len(f.NifCif) = 10 then substring(f.NifCif,1,9) else f.NifCif end,
	Numero_Factura=f.Factura,f.Factura,Moneda=f.DescripcionMoneda,Fecha_Factura=f.FechaFactura,Valor_Total=f.ImportePendiente,IVA=isnull(i.IVA,0),
