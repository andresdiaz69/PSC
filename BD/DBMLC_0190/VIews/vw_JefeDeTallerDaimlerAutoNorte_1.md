# View: vw_JefeDeTallerDaimlerAutoNorte_1
...
CREATE VIEW [dbo].[vw_JefeDeTallerDaimlerAutoNorte_1] AS 
--MODELO SUB 51 --JEFE DE TALLER DAIMLER AUTO NORTE
--MODELO SUB CRITERIO 95 -- Comisión por hora facturada
--IDRANGOMAESTRA 275 JEFE DE TALLER DAIMOER AUTOR NORTE
	...
	(
		...
		(
			...
			(
				...
				WHERE (vw_RangosVersionesMaxSub.IdComisionModeloSub = 58) AND (vw_RangosVersionesMaxSub.IdComisionModeloSubCriterio = 103)
			)AS EMPLEADOS
			LEFT JOIN 
			(
		...
		LEFT JOIN 
		(
			SELECT  Ano_Periodo, Mes_Periodo ,CodigoEmpresa, Empresa, CodigoCentro, Centro, CodigoSeccion, Seccion, TipoOperacion, SUM(UnidadesVendidas) AS UnidadesVendidas
