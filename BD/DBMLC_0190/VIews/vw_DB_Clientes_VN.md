...
--****************************
CREATE view vw_DB_Clientes_VN as
select distinct Codigoempresa,	Empresa,	         TipoDocumento,     nit Cedula,	        nombreTercero NombreCliente,	
 ...
 from (
	  select distinct c.CodigoMArca,
	         ...
	         t.Nifcif,               c.codigocentro,        c.codigogama,
			 c.Empresa,			     c.fechaentregacliente, d.provincia ciudad,
			 v.placa,                v.Combustible,         v.Cilindrada,  
			 t.FkDocumentacionTipos, Servicio,              NumeroMotor,
			 c.PrecioLista,          kmsactuales,           Procedenciadetalle,   
			 cv.NombreClasificacion, color,                 ct.Descripcion CategoriaCliente,
			 t.FkTerceroCargos,      p.Descripcion cargo,   celular1, 
			 totalfactura,
			 case when fkterceroclases = 1 then 'Natural'
	              when fkterceroclases = 2 then 'Empresa'
			      else null end tipoPersona, tdv.TipoVenta,
	    ...
	    from [dbmlc_0190].dbo.ComisionesSpigaVn c
