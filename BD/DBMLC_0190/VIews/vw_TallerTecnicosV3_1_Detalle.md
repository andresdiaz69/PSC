# View: vw_TallerTecnicosV3_1_Detalle
...
```sql
CREATE VIEW [dbo].[vw_TallerTecnicosV3_1_Detalle]
AS
SELECT        dbo.ComisionesSpigaTallerPorOT.Ano_Periodo, dbo.ComisionesSpigaTallerPorOT.Mes_Periodo, 
						 ...
						 SUM(CASE WHEN (dbo.ComisionesSpigaTallerPorOT.FkTarifaTaller IN ('0')) then dbo.ComisionesSpigaTallerPorOT.UnidadesVendidas end) AS UnidadesVendidas_Normal,
...
HAVING   (dbo.vw_RangosVersionesMaxSub.IdComisionModeloSub = 94) AND (dbo.vw_RangosVersionesMaxSub.IdComisionModeloSubCriterio = 175)
