# View: vw_alerta_OTByCargoInformeGlobal

## Usa los objetos:
- [[vw_alerta_OTByCargoInforme]]

```sql

CREATE VIEW [dbo].[vw_alerta_OTByCargoInformeGlobal]
AS
SELECT        Ano_Periodo, Mes_Periodo, IdEmpresas, NombreEmpresa, SiglaEmpresa, CentroSigla, SUM(ValorTotal_Garantia) AS ValorTotal_Garantia, SUM(Cantidad_Garantia) 
...
FROM            dbo.vw_alerta_OTByCargoInforme
--WHERE        (DiasRango > 0)
GROUP BY Ano_Periodo, Mes_Periodo, IdEmpresas, NombreEmpresa, SiglaEmpresa, CentroSigla
