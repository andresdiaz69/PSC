...
- [[vw_VolumenVentasJefeVentasCali]]

```sql
CREATE VIEW [dbo].[vw_JefeDeVentasCali_1] AS
...
(
	...
	FROM
	(
		SELECT Ano_Periodo, Mes_Periodo, Codigoempresa, CodigoCentro, SUM(EntregaEfectiva) AS EntregasEfectivas
		FROM vw_VolumenVentasJefeVentasCali
	...
	(
		...
		(
			...
			WHERE (dbo.vw_RangosVersionesMaxSub.IdComisionModeloSub = 84) AND (dbo.vw_RangosVersionesMaxSub.IdComisionModeloSubCriterio = 161) 
		)AS EMPLEADOS
		LEFT JOIN 
