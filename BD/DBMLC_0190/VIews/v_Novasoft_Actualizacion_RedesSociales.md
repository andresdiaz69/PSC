# View: v_Novasoft_Actualizacion_RedesSociales
...
CREATE view [dbo].[v_Novasoft_Actualizacion_RedesSociales] as
select cedula,fecha
from (
		...
		from(
			...
			from(
					...
					from(
							select fecha=convert(varchar,fecha,23),buscar=charindex('del empleado ',descripcion,25),punto=charindex('.',descripcion,30),
							...
							and descripcion like '%Redes sociales%'
					)a	
			) c 
			where inicio <> 0
		) d
