# View: vw_JefesDeTallerVolkswagen_1
...
CREATE VIEW [dbo].[vw_JefesDeTallerVolkswagen_1]
AS
...
FROM            (SELECT        dbo.ExogenasJefesTallerCT.Ano AS Ano_Periodo, dbo.ExogenasJefesTallerCT.Mes AS Mes_Periodo, dbo.ExogenasJefesTallerCT.CodigoEmpleado, EMPLEADO.IdRangoMaestra, EMPLEADO.IdRangoVersionMax,
                          ...
                          FROM            (SELECT        dbo.Empleados.CodigoEmpleado, dbo.vw_RangosVersionesMaxSub.IdRangoMaestra, dbo.vw_RangosVersionesMaxSub.IdRangoVersionMax, dbo.vw_RangosVersionesMaxSub.IdComisionModeloSub, 
                                                    ...
                                                    WHERE        (dbo.vw_RangosVersionesMaxSub.IdComisionModeloSub = 65) AND (dbo.vw_RangosVersionesMaxSub.IdComisionModeloSubCriterio = 120)) AS EMPLEADO LEFT OUTER JOIN
...
WHERE        (dbo.vw_RangosMaestrasFull.IdComisionModeloSub = 65) AND (dbo.vw_RangosMaestrasFull.IdComisionModeloSubCriterio = 120) AND (dbo.vw_RangosMaestrasFull.Desde < ROTACION.Rotacion) AND 
