# View: vw_Terceros_Registro_Unico_DANIEL
...
CREATE view [dbo].[vw_Terceros_Registro_Unico_DANIEL] as
...
----DC: 2023/12/13 -- Se incorpora dentro de una subconsulta para evitar duplicidad de datos y poder agregarle la llave para mapearla en el entity

SELECT   ROW_NUMBER() OVER(ORDER BY A.pkterceros) Id, 
* FROM(
  select distinct 
  --orden = ROW_NUMBER() over(partition by t.nifcif order by t.FechaAlta desc),
         ...
         from 
	    (select NifCif, max(FechaAlta)FechaAlta
