# Stored Procedure: sp_GMC_JD_UsoCFC_Total1
...
- [[GMF_CFC]]
- [[GMF_JD_ingresoCFC]]
- [[v_GMC_JD_UsoCFC_Total]]
- [[v_GMF_1_JD_ValoresVentasNetas]]
- [[v_GMF_JD_UsoCFC]]

```sql
CREATE PROCEDURE [dbo].[sp_GMC_JD_UsoCFC_Total1] --2021,3,2
...
--declare @anio varchar(5), @mes varchar(5), @CodPresentacion varchar(5)
--set @anio =2021
--set @mes =3
--set @CodPresentacion =72


--drop table #Temp
--drop table #Temp1
--drop table #PorcUsoCFC
--drop table #usocfc

truncate table GMC_JD_UsoCFC_TotalInt
...
Sede,
valor * (select round((((429902418.00 * PorcDistribucion)*0.6/100)*0.6/100),0) from [dbo].[GMF_JD_ingresoCFC] where CodigoPresentacion = a.CodigoPresentacion) as Valor,1 as N1, 0 as N2 
into #temp1
from #Temp as a
...
select CodigoPresentacion,round(((429902418.00 * PorcDistribucion)*0.4/100),0) as valorcfc
into #usocfc
from [dbo].[GMF_JD_ingresoCFC] 
where año = @anio and mes = @mes-1 and CodigoPresentacion = @CodPresentacion


INSERT INTO GMC_JD_UsoCFC_TotalInt
select * 
from (
select Empresa,
CodigoPresentacion,
...
Sede,
isnull(round(valor * (select valor from GMC_JD_UsoCFC_Total where codigopresentacion = b.codigopresentacion and NombreConcepto like '%Porcentaje Taller%' and CodigoConcepto in (53) and sede = b.Sede )/100,0),0) as valor,1 as N1, 0 as N2 
...
Sede,
isnull(round(valor * (select valor from GMC_JD_UsoCFC_Total where codigopresentacion = b.codigopresentacion and NombreConcepto like '%Porcentaje Repuestos%' and CodigoConcepto in (52) and sede = b.Sede )/100,0),0),1 as N1, 0 as N2 
from #temp1 as b --where sede = 'JD-Monteria-Via Cereté' 
union all
select Empresa,
CodigoPresentacion,
...
Sede,
isnull(round(valor * (select valor from GMC_JD_UsoCFC_Total where codigopresentacion = b.codigopresentacion and NombreConcepto like '%Porcentaje Vehiculos%' and CodigoConcepto in (18) and sede = b.Sede )/100,0),0),1 as N1, 0 as N2 
...
from GMC_JD_UsoCFC_Total where nombreconcepto like '%Total UsoCFC %'
...
,sum(a.valor + b.valor ),
a.n1,a.n2
from GMC_JD_UsoCFC_Total as a
join GMC_JD_UsoCFC_TotalInt as b on 
...
select * from v_GMC_JD_UsoCFC_Total
