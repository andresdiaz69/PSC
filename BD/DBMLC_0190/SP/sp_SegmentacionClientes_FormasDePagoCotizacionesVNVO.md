# Stored Procedure: sp_SegmentacionClientes_FormasDePagoCotizacionesVNVO

## Usa los objetos:
- [[Centros]]
- [[Empresas]]
- [[spiga_FormasDePagoCotizacionesVNVO]]
- [[spiga_Terceros]]
...
-- =============================================
CREATE PROCEDURE [dbo].[sp_SegmentacionClientes_FormasDePagoCotizacionesVNVO]
...
PAGOS.IdTerceros, 
PAGOS.IdTercerosRecibo AS IdTercerosPagador,

TERCEROS.NifCif,
TERCEROS.Tercero,

PAGOS.IdRecibos AS NumeroDocumento, 
PAGOS.Fecha AS FechaDocumento,
PAGOS.Fecha AS FechaCancelacion,
PAGOS.DescripcionPagosTipos AS FormaPago,
PAGOS.Importe,
...
DIRECCIONES.Provincia, 
						 ...
						 CASE 
						 WHEN IdTerceros <> IdTercerosRecibo THEN 3
						 ...
						 FROM            (SELECT        PkFkTerceros, MAX(FkProvincias) AS FkProvincias, MAX(Provincia) as Provincia
						  ...
						  RIGHT OUTER JOIN
                             ...
                             (SELECT        MAX(IdSincronizacionSpiga) as IdSincronizacionSpiga, MAX(IdConsecutivo) as IdConsecutivo , MAX(Ano_Periodo) as Ano_Periodo, MAX(Mes_Periodo) as Mes_Periodo, IdEmpresas, Empresas.NombreEmpresa, Empresas.SiglaEmpresa, IdCentros, Centros.NombreCentro, IdTerceros, IdTercerosRecibo, IdRecibos, Fecha, MAX(IdPagoTipos) AS IdPagoTipos, MAX(DescripcionPagosTipos) AS DescripcionPagosTipos, SUM(Importe) AS Importe
                               FROM            
							   
								[PSCService_DB].dbo.spiga_FormasDePagoCotizacionesVNVO
								LEFT OUTER JOIN
								Centros ON [PSCService_DB].dbo.spiga_FormasDePagoCotizacionesVNVO.IdCentros = Centros.CodigoCentro 
								LEFT OUTER JOIN
								Empresas ON [PSCService_DB].dbo.spiga_FormasDePagoCotizacionesVNVO.IdEmpresas = Empresas.CodigoEmpresa

							   WHERE IdRecibos IS NOT NULL
							   GROUP BY Ano_Periodo, Mes_Periodo, IdEmpresas, Empresas.NombreEmpresa, Empresas.SiglaEmpresa, IdCentros, Centros.NombreCentro, IdTerceros, IdTercerosRecibo, IdRecibos, Fecha) AS PAGOS ON TERCEROS.PkTerceros = PAGOS.IdTerceros

							   where PAGOS.Fecha >= @FechaDesde AND PAGOS.Fecha <= @FechaHasta
