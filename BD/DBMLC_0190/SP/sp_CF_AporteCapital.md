# Stored Procedure: sp_CF_AporteCapital
...
CREATE PROC [dbo].[sp_CF_AporteCapital]
@FechaConsulta DATE,
@FechaIngreso DATE
AS
BEGIN
	...
	IF OBJECT_ID(N'tempdb.dbo.#TempAporteCapital', N'U') IS NOT NULL
		DROP TABLE #TempAporteCapital

	SELECT	IdEmpresa=A.IdEmpresas		,IdCentro = A.Centro
			,NombreCentro				,Debe=ISNULL(Debe,0)		
			,Haber=ISNULL(Haber,0)
	INTO #TempAporteCapital
	...
	WHERE Ano_Periodo = YEAR(@FechaConsulta) AND MONTH(FechaAsiento) <= MONTH(@FechaConsulta) 
		AND Cuenta = '3315350505' AND IdEmpresas IN (1, 5, 6, 19, 20, 24, 25, 27)
