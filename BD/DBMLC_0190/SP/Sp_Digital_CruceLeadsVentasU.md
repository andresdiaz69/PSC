# Stored Procedure: Sp_Digital_CruceLeadsVentasU
...
CREATE PROC [dbo].[Sp_Digital_CruceLeadsVentasU]
						...
						FROM(
							--cruzar leads seleccionados previamente contra comisiones vn en el mes seleccionado
								...
								FROM  ComisionesSpigaVO v,
									...
									--	   and s.FechaBaja is null
									--	   and (l.fecha between @fechaini and @fechafin)
									    )a , vw_Terceros_consolidado s WHERE a.PkFkTerceros = s.PkTerceros 
									)l WHERE v.Nit = l.NifCif 
									     AND (YEAR(FechaEntregaCliente)=@ano_EntregaVehiculo 
										 AND MONTH(FechaEntregaCliente)=@mes_EntregaVehiculo)
							)ca, Leads l WHERE ca.id_av = l.id_av
					)cb
			)res WHERE res.Fila = 1
	)reporte WHERE PrecioVehiculo >= 0
END
