# Stored Procedure: sp_ComisionesManualesInformesLiquidacionesPendientes
...
CREATE PROCEDURE com.sp_ComisionesManualesInformesLiquidacionesPendientes   
...
--set @IdTipoEsquema = 1
	...
	CREATE TABLE #NovedadesExogenasReporteLiquidacion (Idregistro int , IdRangoMaestra smallint   ,IdEsquema smallint   ,IdComisionModelo smallint   ,CodigoEmpleado bigint
	...
	INSERT INTO #NovedadesExogenasReporteLiquidacion
	...
	SELECT ISNULL(ROW_NUMBER() OVER(ORDER BY Ano_Periodo, Mes_Periodo, cmn.CodigoEmpleado ASC), 0) AS IdConsecutivo,
	   ...
	   rmv.ConsecutivoVersion, cmn.ValorNovedad       ValorNovedadCriterio1,     cmn.ValorNovedad2        ValorNovedadCriterio2,
		...
		CONVERT(DATE, cmn.FechaModificacion) FechaNovedad					  
	  FROM #NovedadesExogenasReporteLiquidacion as cmn 
	  ...
	  WHERE		cmn.CodigoEmpleado + cmn.IdRangoMaestra + cmn.Ano_Periodo + cmn.Mes_Periodo not in 
				(select cmld.CodigoEmpleado + cmn.IdRangoMaestra + cmld.Ano_Periodo + cmld.Mes_Periodo
				from	com.ComisionesManualesLiquidacionesDetalles cmld where cmld.Anulada = 0)
			and cmn.Ano_Periodo = @Ano_Periodo and cmn.Mes_Periodo = @Mes_Periodo and cmn.IdComisionModelo = @IdDepartamento
	 ...
	 GROUP BY cmn.Ano_Periodo,       cmn.Mes_Periodo,         cmn.CodigoEmpleado,      cmn.IdRangoMaestra,    cmn.Ano_Periodo2,      
			  ...
			  crmm.Apellidos,        cmn.FechaModificacion
	...
	SELECT ISNULL(ROW_NUMBER() OVER(ORDER BY Ano_Periodo, Mes_Periodo, cmn.CodigoEmpleado ASC), 0) AS IdConsecutivo,
	   ...
	   rmv.ConsecutivoVersion,case when cmn.ValorNovedad2 is null     then cmn.ValorNovedad              else cmn.ValorNovedad2              end ValorNovedadCriterio1,     cmn.ValorNovedad2        ValorNovedadCriterio2,
							  case when rmd.Desde is null             and hmd.Desde is null              and mmd.DesdeCriterio1 is null      then vmd.Desde
	                               when vmd.Desde is null             and hmd.Desde is null              and mmd.DesdeCriterio1 is null      then rmd.Desde          
								   when vmd.Desde is null             and rmd.Desde is null              and mmd.DesdeCriterio1 is null      then hmd.Desde             
	 ...
	 DROP TABLE #NovedadesExogenasReporteLiquidacion;
