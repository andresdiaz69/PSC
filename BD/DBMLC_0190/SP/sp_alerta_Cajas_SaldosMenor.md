# Stored Procedure: sp_alerta_Cajas_SaldosMenor
...
-- Create date: <24 Julio 2023>
-- Description:	<Saldo diario de cajas para alerta>
-- =============================================
CREATE PROCEDURE [dbo].[sp_alerta_Cajas_SaldosMenor]
...
FROM    (SELECT        
		...
		and IdEmpresas = @empresa)  AS PERIODO_ACT 
		FULL OUTER JOIN
		...
		)	AS PERIODO_ANT ON	PERIODO_ACT.IdTipos = PERIODO_ANT.IdTipos 
...
where ISNULL(PERIODO_ACT.IdTipos, PERIODO_ANT.IdTipos) = 2
group by 
