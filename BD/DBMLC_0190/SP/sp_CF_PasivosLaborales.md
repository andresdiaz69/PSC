# Stored Procedure: sp_CF_PasivosLaborales
...
CREATE PROC [dbo].[sp_CF_PasivosLaborales]
	@FechaConsulta DATE,
	@FechaIngreso DATE
AS
BEGIN
	...
	IF OBJECT_ID(N'tempdb.dbo.#TempPasivosLaborales', N'U') IS NOT NULL
		DROP TABLE #TempPasivosLaborales

	SELECT	 IdEmpresa=A.IdEmpresas			,IdCentro = A.Centro			,NombreCentro
			,Debe=ISNULL(Debe,0)			,Haber=ISNULL(Haber,0)
	INTO #TempPasivosLaborales
	...
	WHERE Ano_Periodo = YEAR(@FechaConsulta) AND MONTH(FechaAsiento) <= MONTH(@FechaConsulta) 
		AND ((Cuenta LIKE '2505%' AND Cuenta NOT LIKE '250595%') OR (Cuenta LIKE '2610%') OR (Cuenta LIKE '2525%'))
	...
	(
		...
		SELECT	 A.IdEmpresa		,IdLinea = B.Id_Linea
				,A.IdCentro			,Valor=(SUM(Debe) - SUM(Haber)) * -1
		FROM #TempPasivosLaborales A
		LEFT JOIN (
