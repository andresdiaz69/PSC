# Stored Procedure: Sp_Provision_AgregarAResultadoPorcentaje
...
CREATE PROCEDURE Sp_Provision_AgregarAResultadoPorcentaje
...
BEGIN
	...
	BEGIN
		...
		SET @VALOR = (	SELECT TOP 1 C.Valor 
						FROM Provision_OT_Compania C 
						WHERE C.Calculo = 'PORCENTAJE')
		...
		SELECT C.IdCompania, D.IdCentro, D.IdSeccion, D.IdMarca, D.Seccion,
				Total = CONVERT(DECIMAL(18,0),(SUM(D.VrCosteRep + D.VrCosteSub) * C.Valor))
		FROM Provision_OT_Datos D, Provision_OT_Compania C
		WHERE D.IdProvisionCompania = C.IdCompania AND C.Calculo = 'PORCENTAJE'
		GROUP BY C.Valor, C.IdCompania, D.IdCentro, D.IdSeccion, D.IdMarca, D.Seccion 
		...
		WHILE(@A <= 1)--INICIO DEL CICLO PARA RESGISTRO DE PROVISION Y BALANCE
		BEGIN
			PRINT '=> UNA VUELTA'
	...
	ELSE
	BEGIN
		PRINT 'NO ESTAN LAS CUENTAS ADECUADAS PARA LA PROVISION'
	END
